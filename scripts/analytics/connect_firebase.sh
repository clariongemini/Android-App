...
if [[ -z "$SOURCE" ]]; then
  ...
  echo ""
  echo "Firebase Console → Project settings → Your apps → Android (com.konusma) → Download google-services.json"
  exit 1
fi
...
if "com.konusma" not in packages:
    print(f"❌ Expected package com.konusma, found: {packages}")
    sys.exit(1)
...
echo "  3. Confirm 13/13 events → record field proof"
